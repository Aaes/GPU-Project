...
point InnerRK(double h, point p, double eta, double t, double k, double x);
...
{	
  ...
  //Test data
	...
	//point resultPoint = Middle(0.2);

	//point resultPoint = Inner(37.75, 0.2, 0.0);
...
point Middle(double t){
	...
	for(s = 0; s < fullsteps; s++)
	{
		nextPoint = MiddleRK(stepsize, nextPoint, 120.0+s*stepsize, t);
		n += stepsize;
		counter2++;
	}
...
point Inner(double eta, double t, double k){
	...
	double stepsize = -1.0 / h3; //since we are taking steps back the stepsize is negative
	...
	//printf("%.14f\n", ((120.0-eta)+firstStep)+10*stepsize);
	//printf("%.14f\n", nextPoint.x);
