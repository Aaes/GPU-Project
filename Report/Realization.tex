...
Shared Memory/Block*&0 bytes&1904 bytes\\\hline
Global Load Efficiency&90,2\%&764.5\%\\\hline
Global Store Efficiency&99,2\%&25\%\\\hline
Branch Divergence Overhead&22.3\%& 21.8\%\\\hline
Occupancy**&10.5\%&33.1\%\\\hline
\end{tabular}
\end{center}
\caption{Nvidia Profiler Results. Input: g=10, r=65, x=35, h1 = 10, h2 = 2, h3=10. *The visual profiler wrote that OuterPar used 1859 bytes of shared memory. This is however wrong. The command line profiler showed the correct result. The result can be calculated as 119*h2*8, which in this case is 119*2*8 = 1904bytes. **According to the profiler the theoretical occupancy is 33.3\%.}
\label{table:profiler}
\end{table}

\subsubsection{Comparison and discussion} \hfill \\
TODO: Discuss the profiling results\\
TODO: Update: Include the ideas noted down during discussion with HHB\\
TODO: Describe which solution should be the fastest and argue why (Base argument on GPU architecture and results from profiler)
