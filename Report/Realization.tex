...
Furthermore we parallelized the calls to \texttt{Middle}. A closer look at the outer model shows that the different calculations of \texttt{Middle} in every outer step depends only on one variable; the current $x$-value. This effectively meant that we could calculate all $x$-values before the kernel was invoked since the stepsize $h$ and the initial and final $x$-value are known. Using these $x$-values we could calculate all calls to Middle in parallel. Together with the changes to \texttt{Middle} this allowed us to run several blocks to calculate different calls to \texttt{Middle} at the same time and make better use of the GPU resources. The OuterPar implementation is described in the following section.
