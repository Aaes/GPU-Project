...
The differential equation is solved from $t=120-x$ to $t=0$ with the RANDBETINGELSE $f(120-x)=0$
...
Where $g_\tau$ is the propability that a $\tau$ year is married and $f(\eta|\tau)$ is SANDSYNLIGHEDSFORDELINGEN FOR that a $\tau$ year old is married to a $\eta$ year provided that the $\tau$ year old is married.
...
This differential equation is solved from $\eta = 120$ to $\eta = 1$ with the RANDBETINGELSE $f(120) = 0$
...
This differential equation is solved from $s = 120 - \eta$ to $s = 0$ with the RANDBETINGELSE $f(120 - \eta) = 0$
